const { OperationType } = require("@azure/cosmos");
const {
  conectionAzureCosmosDB,
} = require("../../config/conectionToAzureCosmosDB"); // Asegúrate de que esté correctamente importado
...
async function GetAllPricesHistoryCosmos(req) {
  try {
    ...
    const { resources: pricesHistory } = await container.items
      .query("SELECT * FROM c")
      .fetchAll();
...
async function AddManyPricesHistoryCosmos(req) {
  try {
    ...
    if (!Array.isArray(prices) || prices.length === 0) {
      req.error(
        400,
        "El array de precios es requerido y no puede estar vacío."
      );
      return;
    }
    ...
    const formattedPrices = prices.map((price) => {
      ...
      if (missingFields.length > 0) {
        throw new Error(
          `Campos requeridos faltantes en un registro: ${missingFields.join(
            ", "
          )}`
        );
      }
      ...
      return {
        ...
        DATE:
          typeof price.DATE === "string"
            ? new Date(price.DATE).toISOString()
            : price.DATE,
      };
    });
    ...
    const successfulItems = response
      .filter((res) => res.statusCode === 201)
      .map((res) => {
        const { _rid, _self, _etag, _attachments, _ts, ...cleanItem } =
          res.resourceBody;
        return cleanItem;
      });
